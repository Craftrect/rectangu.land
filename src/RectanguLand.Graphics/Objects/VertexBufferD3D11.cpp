...
namespace Graphics
{
	VertexBufferD3D11::VertexBufferD3D11(Device::GxDeviceD3D11Ptr device) : VertexBuffer(ApiType::D3D11) {
		mDevice = device;
	...
	void VertexBufferD3D11::setData(const void* data, uint32 size) {
		if (size == 0) {
			throw std::invalid_argument("Invalid size for vertex buffer: 0");
		}
	...
	void VertexBufferD3D11::recreateBuffer(const void* initialData) {
